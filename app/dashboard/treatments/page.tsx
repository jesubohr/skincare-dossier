    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Manage the service menu offered to your clients</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={handleSeed} disabled={seeding}>
            <ListPlus className="h-4 w-4" />
            {seeding ? "Adding..." : "Add common treatments"}
          </Button>
          <TreatmentSheet />
        </div>
      </div>

      {isLoading ? <p className="text-sm text-muted-foreground">Loading treatments...</p> : <TreatmentsTable treatments={treatments ?? []} />}
    </div>
  )
}
